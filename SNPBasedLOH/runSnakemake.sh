...
#run entire pipeline
snakemake --executor cluster-generic --cluster-generic-submit-cmd "sbatch -N {resources.nodes} -p {resources.slurm_partition} {resources.slurm_extra} --parsable" \
...
snakemake -R --until "INSERT TARGET RULE HERE" --executor cluster-generic --cluster-generic-submit-cmd "sbatch -N {resources.nodes} -p {resources.slurm_partition} {resources.slurm_extra} --parsable" \
--cluster-generic-cancel-cmd scancel --use-conda --jobs 30 --immediate-submit --notemp --printshellcmds --latency-wait 21600
#dry run
snakemake --executor cluster-generic --cluster-generic-submit-cmd "sbatch -N {resources.nodes} -p {resources.slurm_partition} {resources.slurm_extra} --parsable" \
--cluster-generic-cancel-cmd scancel --use-conda --jobs 30 --immediate-submit --notemp --printshellcmds --latency-wait 21600 -n

#add if issues with rerunning complete jobs
--rerun-triggers mtime
--rerun-incomplete

#add if some jobs will fail
--keep-going

#to force execution (use in conjunction with --until)
-F

#for uniquely named snakefile
-s
