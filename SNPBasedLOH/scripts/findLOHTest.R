...
if(nrow(sampDepths) - length(dropSNPs) == 0){
  ...
  if(length(fixedDiff) == 0){
    ...
    if("matrix" %in% class(gts[unlist(fixedDiff),])){
      ...
      fixedDiffADs <- extract.gt(datFilt,element="AD")[unlist(fixedDiff),]
    } else {
      ...
      fixedDiffADs <- matrix(extract.gt(datFilt,element="AD")[unlist(fixedDiff),],
                             ncol=ncol(gts))
    ...
    for(j in 1:nrow(fixedDiffGts)){
      ...
      #extract pfor genotypes and allele depths
      pforGTs <- strsplit(as.vector(fixedDiffGts[j,-c(pmex,plat)]),split="|")
      pforADs <- strsplit(as.vector(fixedDiffADs[j,-c(pmex,plat)]),split=",")
      ...
      for(k in 1:length(pforGTs)){
        ...
        } else if(length(unique(pforGTs[[k]])) == 2 &&
                  !(0 %in% pforADs[[k]])){
          print(paste0("mismatch gt/ad"))
          nMisGTAD <- nMisGTAD+1
        }
      }
    ...
    print(paste0("Mismatches: ",nMismatch))
