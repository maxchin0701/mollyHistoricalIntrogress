...
modelsNeat <- c("No gene flow",
                "Modern gene flow",
                "Ancestral gene flow",
                "Modern + ancestral gene flow (constant rate)",
                "Modern + ancestral gene flow (rate shift)")

#load in lhoods
lhoods <- as.data.frame(matrix(NA,nrow=0,ncol=2))

for(i in models){
  curLhood <- read.delim(paste0("../outputs/lhoods/",i,".lhoods"),
...
lhoodsBest <- lhoods[which(lhoods$model=="No gene flow" |
...
#### PLOT ####
cols <- colorRampPalette(c("#F37221","#0000FF"))(2)

ggplot()+
  geom_density(aes(x=lhoodsBest$lhood,fill=lhoodsBest$model))+
