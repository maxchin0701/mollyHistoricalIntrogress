...
LOHRegions <- read.delim(paste0("../../SNPBasedLOH/output/LOHRegions/LOHRegionsAll.tsv"),
                         sep="\t")
LOHRegionsLong <- LOHRegions[which(LOHRegions$intSize > 1000 & (LOHRegions$group >= 18 | LOHRegions$group <= 6)),]
chrs <- read.delim(paste0("../../SNPBasedLOH/data/chrIndex.tsv"),
                   sep="\t",row.names = NULL,header = T)

#### GET SHARED VS ISOLATED ####
LOHRegionsLong$freq <- apply(LOHRegionsLong,1,function(x) if(as.numeric(x["group"]) <= 6){
  return("Isolated")
  } else {
    return("Shared")
  })
...
#plot
pdf(file=paste0("suppFigTractSize.pdf"),
